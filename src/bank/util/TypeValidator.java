...
public class TypeValidator {
    ...
    public static void validateId(String description, int id) {
        if (id <= 0) {
            throw new IllegalArgumentException(
                ...
                " `" +
                id +
                "` is not a valid SQL id (must be > 0)"
            );
        }
    }
    ...
    private static final Pattern NON_EMPTY_STRING = Pattern.compile(
        "^\\S(?:.*\\S)?$"
    );
    ...
    public static void validateNonEmptyText(String description, String text) {
        ...
        if (!NON_EMPTY_STRING.matcher(text).matches()) {
            throw new IllegalArgumentException(
                ...
                " `" +
                text +
                "` is blank or starts and ends with trailing spaces"
            );
        }
    }

    private static final Pattern SOCIAL_INSURANCE_NUMBER = Pattern.compile(
        "^\\d{3}-\\d{3}-\\d{3}$"
    );

    public static void validateSocialInsuranceNumber(
        String description,
    ...
    public static void validatePhone(String description, String phone) {
        ...
        if (!PHONE.matcher(phone).matches()) {
            throw new IllegalArgumentException(
                ...
                " `" +
                phone +
                "` does not match phone number format of `+` followed by 2-15 digits"
            );
        }
    }
    ...
    public static void validatePositiveMoney(
        String description,
        BigDecimal money
    ) {
    ...
    public static void validatePositiveInteger(
        String description,
        int integer
    ) {
        if (integer < 0) {
            throw new IllegalArgumentException(
                description + " `" + integer + "` is not positive"
            );
        }
    }
}
